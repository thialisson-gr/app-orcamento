...
import { buscarRegraPadrao, salvarRegraPadrao } from '../../services/firebase/firestore'; // 👈 NOVO IMPORT
...
export default function ProfileScreen() {
  ...
  // 👇 NOVOS: Estados do Modal de Regra Padrão
  ...
  // 👇 NOVO: Busca a regra salva assim que a tela abre
  useEffect(() => {
  ...
  const handleSalvarRegra = async () => {
    ...
    Alert.alert('Sucesso', 'Regra padrão atualizada com sucesso! Todas as novas contas Comuns usarão este valor.');
  };
  ...
  // ... (Funções de gerarPDF, gerarCSV, filtrarTransacoes, executarExportacao foram mantidas exatamente iguais)
      ...
      for (const [nomeTabela, transacoesDaTabela] of Object.entries(transacoesAgrupadas)) {
        ...
        htmlTabelas += `
          <div style="margin-bottom: 40px;">
            <h2 style="color: #1f2937; margin-bottom: 10px; padding-bottom: 5px; border-bottom: 2px solid #3b82f6;">📘 Tabela: ${nomeTabela}</h2>
            <table>
      ...
      const htmlContent = `
        <html>
          ...
          <body>
            ...
            <div class="header-info">Exportado por: <b>${nomeExibicao}</b> &nbsp;|&nbsp; Período: <b>${periodoTexto}</b> <br/>Gerado em: ${new Date().toLocaleDateString('pt-BR')} às ${new Date().toLocaleTimeString('pt-BR')}</div>
  ...
  const gerarCSV = async (lista: any[]) => {
    ...
    try {
      ...
      listaOrdenada.forEach(t => {
        csvString += `${t.accountId},${t.type},${formatarData(t.paymentDate || t.date)},${t.descricao.replace(/,/g, ' ')},${t.tags ? t.tags[0] : ''},${t.amount.toFixed(2)},${t.isPaid ? 'Pago' : 'Pendente'}\n`;
      });
      ...
      Alert.alert('Erro ao exportar', error.message || 'Falha desconhecida.');
    } finally {
      setIsExporting(false);
    }
  };
  ...
  const filtrarTransacoes = () => {
    ...
    return transacoes.filter(t => {
      ...
      if (dataInicio && dataInicio.length === 10) {
        ...
        inicioValido = dataTransacao >= new Date(Number(anoI), Number(mesI) - 1, Number(diaI));
      }
      if (dataFim && dataFim.length === 10) {
        const [diaF, mesF, anoF] = dataFim.split('/');
        fimValido = dataTransacao <= new Date(Number(anoF), Number(mesF) - 1, Number(diaF), 23, 59, 59);
      }
  ...
  const executarExportacao = () => {
    ...
    if (transacoesFiltradas.length === 0) return Alert.alert('Aviso', 'Nenhuma transação encontrada neste período.');
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.menuGroup}>
          <View style={styles.menuItem}>
            <View style={styles.menuItemLeft}>
              <View style={[styles.menuIconBox, { backgroundColor: '#f3f4f6' }]}><Ionicons name="moon" size={20} color="#4b5563" /></View>
        ...
        <View style={styles.menuGroup}>
          ...
          {/* 👇 Botão que abre o modal da regra */}
          <TouchableOpacity style={styles.menuItem} onPress={() => setModalRegraVisible(true)} activeOpacity={0.7}>
            <View style={styles.menuItemLeft}>
              <View style={[styles.menuIconBox, { backgroundColor: '#eff6ff' }]}><Ionicons name="people" size={20} color="#3b82f6" /></View>
          ...
          <TouchableOpacity style={styles.menuItem} onPress={() => { if (transacoes.length === 0) return Alert.alert('Aviso', 'Sem dados.'); setModalVisible(true); }} activeOpacity={0.7} disabled={isExporting}>
            <View style={styles.menuItemLeft}>
              <View style={[styles.menuIconBox, { backgroundColor: '#ecfdf5' }]}>{isExporting ? <ActivityIndicator color="#10b981" /> : <Ionicons name="download" size={20} color="#10b981" />}</View>
        ...
        <View style={styles.menuGroup}>
          <TouchableOpacity style={styles.menuItem} onPress={handleLogout} activeOpacity={0.7}>
            <View style={styles.menuItemLeft}>
              <View style={[styles.menuIconBox, { backgroundColor: '#fef2f2' }]}><Ionicons name="log-out" size={20} color="#ef4444" /></View>
      ...
      <Modal animationType="slide" transparent={true} visible={modalRegraVisible} onRequestClose={() => setModalRegraVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>Regra de Divisão Padrão</Text>
            <Text style={styles.helperText}>Ajuste a porcentagem padrão para criação de Contas Comuns.</Text>

            <View style={{ backgroundColor: '#f9fafb', padding: 20, borderRadius: 16, marginBottom: 24 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                <Text style={{ fontSize: 16, fontWeight: '600', color: '#1f2937' }}>Sua Parte</Text>
                <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#fff', borderRadius: 8, borderWidth: 1, borderColor: '#e5e7eb' }}>
                  <TouchableOpacity onPress={() => regraEu > 0 && setRegraEu(regraEu - 1)} style={{ padding: 10 }}><Ionicons name="remove" size={20} color="#3b82f6"/></TouchableOpacity>
                  <Text style={{ width: 40, textAlign: 'center', fontSize: 16, fontWeight: 'bold' }}>{regraEu}%</Text>
                  <TouchableOpacity onPress={() => regraEu < 100 && setRegraEu(regraEu + 1)} style={{ padding: 10 }}><Ionicons name="add" size={20} color="#3b82f6"/></TouchableOpacity>
                </View>
              </View>
              ...
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                <Text style={{ fontSize: 16, fontWeight: '600', color: '#1f2937' }}>Parte da Esposa</Text>
                <Text style={{ fontSize: 16, fontWeight: 'bold', color: '#6b7280', marginRight: 16 }}>{regraRay}%</Text>
              </View>
            </View>

            <View style={styles.modalFooter}>
              <TouchableOpacity style={styles.cancelBtn} onPress={() => setModalRegraVisible(false)}><Text style={styles.cancelBtnText}>Cancelar</Text></TouchableOpacity>
              <TouchableOpacity style={styles.confirmBtn} onPress={handleSalvarRegra}><Text style={styles.confirmBtnText}>Salvar Padrão</Text></TouchableOpacity>
            </View>
          </View>
        </View>
      </Modal>

      {/* MODAL DE EXPORTAÇÃO (MANTIDO) */}
      <Modal animationType="slide" transparent={true} visible={modalVisible} onRequestClose={() => setModalVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <View style={styles.row}>
              <View style={[styles.formGroup, { flex: 1 }]}><Text style={styles.label}>Data Inicial</Text><TextInput style={styles.input} placeholder="DD/MM/AAAA" keyboardType="numeric" maxLength={10} value={dataInicio} onChangeText={(t) => handleDataChange(t, setDataInicio)} /></View>
              <View style={[styles.formGroup, { flex: 1 }]}><Text style={styles.label}>Data Final</Text><TextInput style={styles.input} placeholder="DD/MM/AAAA" keyboardType="numeric" maxLength={10} value={dataFim} onChangeText={(t) => handleDataChange(t, setDataFim)} /></View>
            </View>
