...
export default function AccountsScreen() {
        ...
        { 
          ...
          onPress: () => router.push({
            ...
            params: {
              ...
              donoOriginal: conta.dono || '',
              porcentagemOriginal: conta.splitRule.me.toString()
            }
          })
        },
        { text: 'Excluir Tabela', style: 'destructive', onPress: () => confirmarExclusao(conta) }
      ]
    );
  };

  const confirmarExclusao = (conta: any) => {
    Alert.alert('Atenção MÁXIMA!', `Tem certeza que deseja excluir "${conta.nome}"?`, [
      { text: 'Cancelar', style: 'cancel' },
      { text: 'Sim, Excluir', style: 'destructive', onPress: async () => await deletarContaNoFirebase(conta.id) }
    ]);
  };

  // 👇 FUNÇÃO BLINDADA PARA ABRIR A CONTA
  const abrirConta = (nome: string) => {
    router.push(`/account/${encodeURIComponent(nome)}`);
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}><Text style={styles.headerTitle}>Minhas Tabelas</Text></View>
      <View style={styles.monthSelectorContainer}>
        <TouchableOpacity onPress={irMesAnterior} style={styles.monthArrow}><Ionicons name="chevron-back" size={20} color="#3b82f6" /></TouchableOpacity>
        <Text style={styles.monthTextBig}>{mesFormatado}</Text>
        <TouchableOpacity onPress={irProximoMes} style={styles.monthArrow}><Ionicons name="chevron-forward" size={20} color="#3b82f6" /></TouchableOpacity>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
              ...
              return (
                <View key={conta.id} style={styles.accountCard}>
                  <View style={styles.accountHeader}>
                    <TouchableOpacity style={styles.accountTitleRow} activeOpacity={0.6} onPress={() => abrirConta(conta.nome)}>
                    ...
                    <TouchableOpacity style={styles.optionsBtn} onPress={() => handleOpcoesConta(conta)}><Ionicons name="ellipsis-horizontal" size={24} color="#9ca3af" /></TouchableOpacity>
                  </View>
                  <TouchableOpacity style={styles.accountBody} activeOpacity={0.6} onPress={() => abrirConta(conta.nome)}>
                    ...
                    <View style={styles.amountColRight}><Text style={styles.amountLabel}>A Receber</Text><Text style={[styles.amountValue, aReceber > 0 ? { color: '#f59e0b' } : { color: '#10b981' }]}>R$ {aReceber.toFixed(2)}</Text></View>
                  </TouchableOpacity>
                </View>
              );
            })}
        ...
        <View style={styles.section}>
          ...
          {contasDespesa.length === 0 ? <Text style={styles.emptyText}>Nenhuma tabela de despesa criada.</Text> : (
              ...
              return (
                <View key={conta.id} style={styles.accountCard}>
                  ...
                  <TouchableOpacity style={styles.accountBody} activeOpacity={0.6} onPress={() => abrirConta(conta.nome)}>
                    ...
                    <View style={styles.amountColRight}><Text style={styles.amountLabel}>Pendente</Text><Text style={[styles.amountValue, pendente > 0 ? { color: '#ef4444' } : { color: '#10b981' }]}>R$ {pendente.toFixed(2)}</Text></View>
                  </TouchableOpacity>
                </View>
              );
            })
      ...
      <TouchableOpacity style={styles.fab} activeOpacity={0.8} onPress={() => router.push('/add-account')}><Ionicons name="add" size={32} color="#fff" /></TouchableOpacity>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  scrollContent: { padding: 20, paddingBottom: 120 }, section: { marginBottom: 24 }, sectionTitle: { fontSize: 16, fontWeight: 'bold', color: '#6b7280', marginBottom: 16, marginLeft: 4 },
  ...
  accountName: { fontSize: 18, fontWeight: 'bold', color: '#1f2937' }, optionsBtn: { padding: 4, marginLeft: 8 },
  accountBody: { flexDirection: 'row', justifyContent: 'space-between' }, amountCol: { flex: 1 }, amountColRight: { flex: 1, alignItems: 'flex-end' },
